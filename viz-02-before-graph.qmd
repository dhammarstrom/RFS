...
lst <- list(x = c("1"), y = c("1"))

vlst <- Venn(lst)
edge <- process_data(vlst)

dat <- edge$setEdge


# Intersection calculation 
# The intersection can be calculated using the center of two circles
# and their radius. From the setEdge data we get coordinates for the 
# circles.
inter_dat <- function(dat) {
  
  center <- dat |> 
    summarise(.by = id, 
              center_x = mean(X), 
              center_y = mean(Y), 
              radius = mean(sqrt((X - mean(X))^2 + (Y - mean(Y))^2)),
              ) 
  
  d <- sqrt((center$center_x[2] - center$center_x[1])^2 +
  (center$center_y[2] - center$center_y[1])^2)
  
  a <- (center$radius[1]^2 - center$radius[2]^2 + d^2) / (2 * d)
  h <-sqrt(center$radius[1]^2 - a^2) 
  
    # Point on line between centers
  px <-center$center_x[1] + a * (center$center_x[2] - center$center_x[1]) / d
  py <-center$center_y[1] + a * (center$center_y[2] - center$center_y[1]) / d
  
  # The two intersection points
  x_int1 <- px + h * (center$center_y[2] - center$center_y[1]) / d
  y_int1 <- py - h * (center$center_x[2] - center$center_x[1]) / d
  
  x_int2 <- px - h * (center$center_y[2] - center$center_y[1]) / d
  y_int2 <- py + h * (center$center_x[2] - center$center_x[1]) / d
  ...
  circle1_data <- dat[dat$id == 1, ]
  circle2_data <- dat[dat$id == 2, ]
  
  x1 <- mean(circle1_data$X)
  y1 <- mean(circle1_data$Y)
  x2 <- mean(circle2_data$X)
  y2 <- mean(circle2_data$Y)
  
  # Radius is roughly the distance from center to any point
  r1 <- sqrt((circle1_data$X[1] - x1)^2 + (circle1_data$Y[1] - y1)^2)
  r2 <- sqrt((circle2_data$X[1] - x2)^2 + (circle2_data$Y[1] - y2)^2)
  

  # Calculate intersection points
   circle1_in_circle2 <- circle1_data[
    sqrt((circle1_data$X - x2)^2 + (circle1_data$Y - y2)^2) <= r2 + 1e-6, 
  ]
  
  # Points from circle 2 that are inside circle 1
  circle2_in_circle1 <- circle2_data[
    sqrt((circle2_data$X - x1)^2 + (circle2_data$Y - y1)^2) <= r1 + 1e-6, 
  ]
  
  if (nrow(circle1_in_circle2) > 0 && nrow(circle2_in_circle1) > 0) {
    # Order points by angle to create proper polygon
    all_points <- rbind(circle1_in_circle2[, c("X", "Y")], 
                       circle2_in_circle1[, c("X", "Y")])
    
    # Remove duplicates and order by angle from centroid
    centroid_x <- mean(all_points$X)
    centroid_y <- mean(all_points$Y)
    
    all_points$angle <- atan2(all_points$Y - centroid_y, all_points$X - centroid_x)
    all_points <- all_points[order(all_points$angle), ]
...
# specify fill colors
join_illustration <- function(dat, 
                              fills = c("steelblue", 
                                        "white"),
                              fill_intersection = "black",
                              
                              title = "left_join(x,y)") {
  

  dat |> 
  ggplot(aes(Y, X, group = id, fill = id)) + 
  geom_polygon() +
  
  scale_fill_manual(values = fills) +
  
  geom_polygon(data = interdat, aes(group = NULL), 
               fill = fill_intersection) +
  
  geom_path(color = "black") +  # Add black outline
  coord_equal() + 
...
left <- join_illustration(dat, 
                  title = "left_join(x,y)", 
                  fills = c("steelblue", "white"), 
                  fill_intersection = "steelblue")
right <- join_illustration(dat,
                  title = "right_join(x,y)", 
                   fills = c("white", "steelblue"), 
                  fill_intersection = "steelblue")
inner <- join_illustration(dat,
                  title = "inner_join(x,y)", 
                   fills = c("white", "white"), 
                  fill_intersection = "steelblue")
full <- join_illustration(dat,
                  title = "full_join(x,y)", 
                   fills = c("steelblue", "steelblue"), 
                  fill_intersection = "steelblue")
anti <- join_illustration(dat, 
                  title = "anti_join(x,y)", 
                  fills = c("steelblue", "white"), 
                  fill_intersection = "white")
