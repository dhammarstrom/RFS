...
participant_volume <- exscidata::dxadata |>  
  ...
  pivot_longer(values_to = "leg", 
               names_to = "volume",  
               cols = multiple:single) |> 
  distinct(participant, volume, leg)


leg_leanmass <- exscidata::dxadata |>   
  select(participant, time, include, 
         starts_with("lean.") & ends_with("_leg")
         ) |> 
